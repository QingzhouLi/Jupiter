...
public class GeoRecommendation {
	public List<Item> recommendItems(String userId, double lat, double lon) {
		...
		Set<String> favariteitenmIds = connection.getFavoriteItemIds(userId);

		// Step 2, get all categories, sort by count
		// {"sports": 5, "music": 3, "art": 2}
		Map<String, Integer> allCatigories = new HashMap<>();

		for (String itemId : favariteitenmIds) {
			Set<String> categories = connection.getCategories(itemId);
			for (String category : categories) {
				allCatigories.put(category, allCatigories.getOrDefault(category, 0) + 1);
			}
		}
		List<Entry<String, Integer>> categoryList = new ArrayList<>(allCatigories.entrySet());
		...
		for (Entry<String, Integer> category : categoryList) {
			...
			for (Item item : items) {
				if (!favariteitenmIds.contains(item.getItemId()) && !visitedItemIds.contains(item.getItemId())) {
